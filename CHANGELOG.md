...
- Podcast 2.0 `podcast:chat`, `podcast:podping`, and `podcast:valueTimeSplit` support (#437): `PodcastChat` (channel-level `PodcastMeta.chat` and item-level `PodcastEntryMeta.chat`), `PodcastMeta.podping_uses_podping`, and `PodcastValueTimeSplit` (`PodcastValue.time_splits`), plus `ParserLimits::max_podcast_chat` (default 20) and `max_podcast_value_time_splits` (default 20). Node.js/Python binding parity for all three new tags is deferred, tracked in #467. **Breaking**: `ParserLimits` is not `#[non_exhaustive]`, so adding `max_podcast_chat` and `max_podcast_value_time_splits` breaks any downstream exhaustive `ParserLimits { .. }` literal construction (this PR itself had to patch `crates/feedparser-rs-py/src/limits.rs` for exactly that reason)
- Item-level `podcast:value`/`podcast:valueTimeSplit` support (#466): `PodcastEntryMeta.value: Option<PodcastValue>` is now populated from `<podcast:value>` when it appears inside `<item>`, reusing the same parsing and `ParserLimits` bounds (`max_value_recipients`, `max_podcast_value_time_splits`) as the channel-level tag added in #437. This was flagged during #465's review as materially limiting, since `podcast:valueTimeSplit` is inherently episode-scoped and real-world feeds are expected to place it inside `<item>` rather than only `<channel>`. Node.js/Python binding parity remains deferred (tracked in #467). **Breaking**: `PodcastEntryMeta` is not `#[non_exhaustive]`, so adding `value` breaks any downstream exhaustive `PodcastEntryMeta { .. }` struct literal or destructuring pattern, same class of break as the `ParserLimits` note above
