...
- `FeedMeta::with_rss_capacity` and `Entry::with_capacity` called `Vec::with_capacity(0)` for their `contributors` field, tripping the new clippy `with_capacity_zero` lint; switched to `Vec::new()`.
